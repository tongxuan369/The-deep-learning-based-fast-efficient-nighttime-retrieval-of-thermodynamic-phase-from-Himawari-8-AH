...
import os
from ResUnet50 import Resnet50_Unet
...
def train(gpu_id, gpu_count):
    ...
    net = Resnet50_Unet(inputchannel=inchannel, outputchannel=outchannel, BN_enable=True, resnet_pretrain=True)
...
def test():
    ...
    ssl._create_default_https_context = ssl._create_unverified_context
    net = Resnet_Unet(inputchannel=inchannel, outputchannel=outchannel, BN_enable=True, resnet_pretrain=True)
    net.to(default_device, dtype=torch.float32)
    ...
    test_loader = Data.DataLoader(dataset=dealDataset_test, batch_size=1, shuffle=False, drop_last=False)
    test_np = []
    real_np = []
